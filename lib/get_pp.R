get_pp_chunk <- function(path="largedata/pp", pattern="PC_.*.csv", chunk_inc=100, imp68){
...
get_pp <- function(path="largedata/pp", pattern="PC_.*.csv", imp){
    files <- list.files(path, pattern, full.names = TRUE)
    ...
    for(j in 1:length(pid)){
        ...
        #pp <- pp[order(pp$chr, pp$chunk, pp$idx),]
        
        if(sum(imp[, pid[j]] == 1) != nrow(pp)){
            stop("!!! unequal number of hetersites !!!") 
        }
        message(sprintf("###>>> biggest chunk number of %s: [ %s ]", pid[j], max(pp$chunk)))
